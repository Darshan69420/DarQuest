...
//   STRAFERS: skittish casters that circle you between spells instead of standing still
//   KITERS:   shamans and cultists that back away when crowded, so you chase or switch target
//   LUNGERS:  pack hunters that cross the last few metres in a sudden burst
...
export class Combat {
  ...
  // Lord Hollowmere's signature: below 60% the ground itself hunts the player. Four grave
  // circles snap to where you stand, one after another. Keep moving.
  bossSignature(e, t) {
    ...
    if (g) {
      if (t >= g.next) {
        ...
        const spell = { id: 'grave_grasp', name: 'Grave Grasp', school: 'umbral', type: 'damage', min: 110, max: 140, pips: 2 };
  ...
  startAttack(e) {
    ...
    if (aoe) {
      ...
      this.world.float(e.model, spell.pips >= 4 ? '❗' : '⚠️', 'status');
