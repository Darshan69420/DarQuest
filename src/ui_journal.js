...
export function openJournal(p, { onTitle }, tab = 'quests') {
  const render = (body) => {
    let inner = '';
    if (tab === 'quests') {
      ...
      const chapter = p.quest.index < 7 ? 1 : p.quest.index < 14 ? 2 : p.quest.index < 23 ? 3 : 4;
      inner = `<h3 class="sub-h">⭐ Main story</h3>
        <div class="journal-q main"><b>${esc(t.title)}</b><br><span>${esc(t.goal)}</span>${main ? `<br><small>Chapter ${chapter} · quest ${p.quest.index + 1} of ${QUESTS.length}</small>` : ''}</div>
    ...
    body.innerHTML = `<div class="tabs">${[['quests', '📜 Quests'], ['bestiary', '🐾 Bestiary'], ['achievements', '🏆 Achievements']].map(([k, l]) => `<button class="btn small ${tab === k ? 'primary' : ''}" data-tab="${k}">${l}</button>`).join('')}</div>${inner}`;
